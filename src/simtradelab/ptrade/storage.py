...
def _date_to_int(dt_series: pd.Series) -> pd.Series:
    ...
    return (
        dt_series.dt.year * 10000 + dt_series.dt.month * 100 + dt_series.dt.day
    ).astype(int)
    ...
    if base_name == "metadata":
        ...
        if ic_file.exists():
            ...
            for date, group in ic_df.groupby("date"):
                index_constituents[date] = dict(
                    zip(group["index_code"], group["symbols"])
                )
...
def load_stock_1m(data_dir, symbol):
    """加载分钟线数据"""
    parquet_file = Path(data_dir) / "stocks_5m" / (symbol + ".parquet")
    if parquet_file.exists():
        df = pd.read_parquet(parquet_file)
        if not df.empty:
...
def list_stocks_1m(data_dir):
    """列出所有可用的分钟数据股票代码"""
    stocks_dir = Path(data_dir) / "stocks_5m"
    if not stocks_dir.exists():
        return []
